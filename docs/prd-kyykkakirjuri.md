...
Kyykkä referees and scorekeepers currently track scores with pen and paper during live games. This is error-prone — especially under time pressure between turns — and makes it hard to get a clear running picture of the game state (who is winning, how many kyykät remain, which erä we're in). Corrections require crossing things out and recalculating. There is no standard tool for the job.

## Solution

A mobile-web app that a referee opRoundtes in the field during a live kyykkä game. It guides the referee through the full game flow: setup (team names, player order) → erä 1 (recording akkat and papit after each turn) → halftime (optional substitutions, side swap) → erä 2 → final results. Scores are calculated automatically. The referee can edit any previously recorded turn at any time if they miscounted.
...
7. As a referee, I want the app to prevent me from entering akkat + papit > 40, so that I catch obvious miscounts before confirming them.
...
11. As a referee, I want to edit any previously recorded turn within the current erä, so that I can fix an error I only noticed later.
12. As a referee, I want the app to detect when a team's target square is fully cleared (akkat = 0, papit = 0) before all 16 kartut are used, so that the field-cleared bonus score is applied automatically.
...
14. As a referee, I want to override the entire erä score with a manually entered final total at any point during the erä, so that I can recover from a sequence of recording errors without tediously correcting each turn individually.
...
16. As a referee, I want to see how many turns remain in the current erä for each team, so that I know how close we are to halftime.
17. As a referee, I want the app to automatically transition to the halftime screen after both teams have completed their 4 turns in erä 1, so that I don't have to navigate manually.
18. As a referee, I want to see the erä 1 scores for both teams on the halftime screen, so that I can announce results before starting erä 2.
...
20. As a referee, I want to update the playing order for each team at halftime, so that the correct player pairs are shown during erä 2.
21. As a referee, I want to confirm side-swap and start erä 2 from the halftime screen, so that the app correctly tracks which team attacks which target square in the second erä.
22. As a referee, I want erä 2 to follow the same turn recording flow as erä 1, so that my workflow is consistent.
23. As a referee, I want the app to automatically show the final results screen after both teams complete their turns in erä 2, so that I don't have to navigate manually.
24. As a referee, I want the final results screen to show each team's erä 1 score, erä 2 score, and total game score, so that I can announce the full result clearly.
...
28. As a referee, I want large, finger-friendly input controls for akkat and papit counts, so that I don't mis-tap under field conditions.
29. As a scorekeeper reviewing the results, I want to see the breakdown of akkat and papit per turn for each team, so that I can verify the score calculation.
...
- `scoreTurn(akkat, papit, kartutUsed, totalKartut)` → `{ points, fieldCleared, unusedKartut }`
- `scoreErä(turns)` → `{ points, fieldCleared }` — applies field-cleared logic: if any turn in the erä reached akkat=0 and papit=0, the erä score is `+unusedKartut` for the remaining kartut at that point; otherwise `sum(akkat × −2) + sum(papit × −1)` at end of erä.
...
- `recordTurn(akkat, papit)` — appends result for the active team/turn; auto-detects field cleared; auto-advances to next team or to halftime/finished
- `editTurn(roundIndex, turnIndex, teamIndex, akkat, papit)` — overwrites a previously recorded result and recalculates downstream scores
- `overrideRoundScore(teamIndex, points)` — replaces calculated erä score with a manually entered value; marks that erä as overridden
- `confirmRoundtauko(substitutions)` — applies substitutions and transitions `halftime → round` (erä 2)
...
- `TurnInputSchema` — `{ akkat: number (0–40), papit: number (0–40) }` with a refinement: `akkat + papit ≤ 40`
...
2. **TurnStep** — akkat/papit spinners, player pair display, live cleared-count indicator, edit history panel
3. **halftimeStep** — erä 1 score summary, substitution form, confirm button
4. **ResultsStep** — full score breakdown, winner declaration, new game button

### i18n

Paraglide.js with Finnish as the default locale and English as a secondary locale. All UI strings — including official kyykkä terms (akka, pappi, turn, erä, etc.) — are in the message catalog. Language toggle is accessible from all steps.
...
- Normal erä: verify that akkat and papit are multiplied by the correct penalty weights and summed.
- Field-cleared bonus: verify that when akkat=0 and papit=0 after turn N, the erä score equals `+(16 − N×4)`.
- Game totals: verify that erä 1 + erä 2 scores sum correctly for both teams.
- Edge cases: all 40 kyykät remaining (maximum penalty), first turn clears everything (maximum bonus), exactly 40 akkat + papit split.

### Game state machine
State transition tests:
- Setup → round transition with valid and invalid inputs.
- turn recording advances to the correct next team.
- After turn 4 of team B in erä 1, state transitions to `halftime`.
- After turn 4 of team B in erä 2, state transitions to `finished`.
- `editTurn` correctly overwrites a prior result and recalculates.
- `overrideRoundScore` marks the erä as overridden and uses the manual value in game totals.
...
### Validation schemas
- `TurnInputSchema` rejects akkat + papit > 40.
