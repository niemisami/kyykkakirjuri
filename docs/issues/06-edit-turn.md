...
The referee can open any previously recorded turn in the current round, correct the akkat or papit count, and see the running score recalculate immediately. This is a correction tool, not a primary input method.
...
2. **Edit in place** — Tapping a history entry opens the same akkat/papit inputs pre-filled with the recorded values. The same `TurnInputSchema` validation applies.
3. **Store: editTurn** — `editTurn(roundIndex, turnIndex, teamIndex, akkat, papit)` overwrites the stored result and recalculates round scores via the scoring engine.
