...
export function QuickGameForm() {
  const form = useForm({
    defaultValues: {
      teamA: {
        name: 'testi',
        players: [{ name: 'Sami' }, { name: 'Sini' }, { name: 'Matu' }, { name: 'Simo-Pekka' }],
      },
      teamB: {
        name: 'testi BB',
        players: [{ name: 'Katariina' }, { name: 'Niklas' }, { name: 'Jerkku' }, { name: 'Dille' }],
      },
    },
    validators: { onSubmit: QuickGameSchema },
    onSubmit: ({ value }) => {
      startGame(
        value.teamA.name,
        value.teamA.players.map(p => ({ name: p.name })),
        value.teamB.name,
        value.teamB.players.map(p => ({ name: p.name })),
        { mode: 'quick' }
      )
    },
  })
