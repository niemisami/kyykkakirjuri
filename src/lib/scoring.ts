 ...
 * @param singleThrowIndexWithinTurn 1..4 single-throw index within the team turn
 */
export function scorePlayerThrow(
  ...
  turnIndex: number,
  totalTurns: number,
  singleThrowIndexWithinTurn: 1 | 2 | 3 | 4
): TurnResult {
  const fieldCleared = akat === 0 && papit === 0
  if(fieldCleared) {
    const remainingFullTurns = totalTurns - turnIndex
    const midTurnBonus = 4 - singleThrowIndexWithinTurn
